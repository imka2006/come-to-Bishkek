<template>
    <form @submit.prevent>
        <div class="send">
            <input type="email" :placeholder="$t('sendInput')" v-model="val" />
            <button type="button" @click="sendEmailToServer()" :disabled="val == ''">{{$t('sendBtn')}} <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
...
const sendEmailToServer = () => {   
  ...
  Email.send({
    ...
    Subject: "Отправка почты",
    Body: `Электронная почта: ${val.value} `,
